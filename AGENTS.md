...
- Portfolio work entries are data-driven from `src/data/works.ts` (a typed `Work[]`). Set `featured: true` to surface a work in the homepage "Selected work" section; every entry also appears on `/works` and gets a detail page at `/works/<slug>` (generated via `getStaticPaths` in `src/pages/works/[slug].astro`).
- Gotcha: after editing `src/data/works.ts`, the running `astro dev` server can serve STALE detail-page (`/works/<slug>`) content because `getStaticPaths` props are cached — the `/works` list may update while detail pages do not. `npm run build` always reflects the latest data; when verifying detail pages in dev, restart the dev server if content looks stale.
